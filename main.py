...
@click.option("--save-profile", required=False, help="Save current arguments as a profile")
...
def run_optout(first, last, email, zip, profile, save_profile, reset):
            ...
            try:
                ...
                if not reset:
                    ...
                    if processed_brokers:
                        print(f"Previously processed brokers for {profile}: {', '.join(processed_brokers)}")
                
                print(f"Loaded profile: {profile}")
            except Exception as e:
                print(f"Error loading profile {profile}: {e}")
        else:
            print(f"Profile {profile} not found. Will create it if --save-profile is used.")
    
    # Check if we need to save this as a new profile
    if save_profile:
        if not all([first, last, email, zip]):
            print("Error: All personal information (first, last, email, zip) must be provided to save a profile.")
            sys.exit(1)
    ...
    if not all([first, last, email, zip]):
        print("Error: Missing required information. Provide all options or use --profile.")
        sys.exit(1)
    ...
    if profile and profile_path and newly_processed_brokers:
        try:
            ...
            user_data["processed_brokers"] = list(set(current_processed))  # Remove duplicates
            ...
            print(f"Updated profile {profile} with {len(newly_processed_brokers)} newly processed brokers")
        except Exception as e:
